...
def read_all_bases():
    ...
    depara_acoes = depara_acoes.with_columns(
        pl.concat_str(['COD_INICIATIVA', 'INICIATIVA'], separator=' - ').alias('depara_acao')
    ...
    precos = pl.read_parquet('data/precos.parquet').select([
        pl.concat_str([
            pl.col('Cob UNB').cast(pl.Utf8),
            pl.col('Cod Prod').cast(pl.Utf8)], sep='_').alias('chave'),
            'Cob UNB', 'Cod Prod', 'Nome Produto', 'TTV CX Frio', 'TTV CX ASR'
        ]).sort(['Cob UNB', 'Cod Prod'])

    bases_foco = pl.read_parquet('data/bloqueios.parquet')

    produtos = produtos.with_columns(
        pl.concat_str([pl.col('cod_sku'),pl.col('nome_sku')], sep = ' - ').alias('cod_nome_sku')
    )

    clientes = clientes.with_columns(
        pl.col('seg_consolidado').fill_null('outros')
    )
...
def check_for_new_file(base_files: str, parquet_files: str) -> bool:
    ...
    if glob.glob(parquet_files):
        ...
        parquet_files_dates = [os.path.getmtime(parquet) for parquet in glob.glob(parquet_files)]
...
def read_excel_parquets(path: str, destino: str) -> None:
    ...
    for sheet in sheets:
        if sheet in ['cerveja', 'depara_repasse', 'repasse']:
...
def fazer_acao(nome_ação, n_acao, cestas, clientes, produtos, bases_foco, depara_acoes):
    ...
    with st.expander(nome_ação, True): 
        ...
        with col1:
            st.multiselect('Cestas', sorted(cestas['Cesta'].unique()), key='cesta_'+str(n_acao))
            
            if st.session_state['cesta_'+str(n_acao)]:
                skus = cestas.filter(pl.col('Cesta').is_in(
                    st.session_state['cesta_'+str(n_acao)])).select(
                        pl.col('cod_sku')
                        ).unique()
                skus = list(list(skus)[0])
                produtos = produtos.filter(pl.col('cod_sku').is_in(skus))
            
            st.multiselect('Marcas', sorted(list(produtos['marca'].unique())), key='marcas_'+str(n_acao))
            ...
            st.multiselect('Embalagens', sorted(produtos['embalagem'].unique()), key='embalagem_'+str(n_acao))
            
            if st.session_state['embalagem_'+str(n_acao)]:
                produtos = produtos.filter(
                    pl.col('embalagem').is_in(st.session_state['embalagem_'+str(n_acao)])
                )            
            
            st.multiselect('Produtos', sorted(produtos['cod_nome_sku'].unique()), key='produto_'+str(n_acao))
            
            if st.session_state['produto_'+str(n_acao)]:
                produtos = produtos.filter(
                    pl.col('cod_nome_sku').is_in(st.session_state['produto_'+str(n_acao)])
                )
            
            if 'selected_'+str(n_acao) not in st.session_state:
                st.session_state['selected_'+str(n_acao)] = "" 
            if produtos.shape[0] <= 100:
                st.session_state['selected_'+str(n_acao)] =  ', '.join([str(x) for x in produtos.to_pandas()['cod_sku'].unique()])
            else:
                st.session_state['selected_'+str(n_acao)] = 'excedendo limite de 100 skus'
            
                
            st.text_area('Custom Filters', value=st.session_state['selected_'+str(n_acao)], key='custom_'+str(n_acao))     
    
        with col2:
            st.multiselect('Comercial', sorted(clientes['comercial'].unique()), key='comercial_'+str(n_acao))
            
            if st.session_state['comercial_'+str(n_acao)]:
                clientes = clientes.filter(
                    pl.col('comercial').is_in(st.session_state['comercial_'+str(n_acao)])
                )
            
            st.multiselect('Operação', sorted(clientes['operacao'].unique()), key='operacao_'+str(n_acao))
            
            if st.session_state['operacao_'+str(n_acao)]:
                clientes = clientes.filter(
                    pl.col('operacao').is_in(st.session_state['operacao_'+str(n_acao)])
                )
            
            st.multiselect('Segmento Consolidado', sorted(clientes['seg_consolidado'].unique()), key='seg_consolidado_'+str(n_acao))
            
            if st.session_state['seg_consolidado_'+str(n_acao)]:
                clientes = clientes.filter(
                    pl.col('seg_consolidado').is_in(st.session_state['seg_consolidado_'+str(n_acao)])
                )
            
            st.multiselect('Segmento', sorted(clientes['segmento'].unique()), key='segmento_'+str(n_acao))
            
            if st.session_state['segmento_'+str(n_acao)]:
                clientes = clientes.filter(
                    pl.col('segmento').is_in(st.session_state['segmento_'+str(n_acao)])
                )
            
            st.radio("Incluir ou Bloquear?", ['Incluir', 'Bloquear'], horizontal=True, key='foco_incluir_'+str(n_acao))
            st.multiselect('Bases Foco', sorted(bases_foco['motivo'].unique()), key='bases_foco_'+str(n_acao))
            
            if st.session_state['bases_foco_'+str(n_acao)]:
                bases_foco = bases_foco.filter(pl.col('motivo').is_in(st.session_state['bases_foco_'+str(n_acao)]))
        ...
        with col3:
            ...
            st.date_input('Data Final', value= datetime.date(ano, mes, calendar.monthrange(ano,mes)[1]), key='data_final_'+str(n_acao))
        ...
        with col4:
            ...
            st.number_input('Max SKU Ação', value = 999999, key = 'max_sku_acao_'+str(n_acao))
            st.number_input('Disponibilidade Pedidos', value = 100, key = 'disp_pedidos_'+str(n_acao))
            st.number_input('Disponibilidade SKU', value = 1000, key = 'disp_sku_'+str(n_acao))     

        with col5:
            st.selectbox('Origem verba', ['1 - GEO', '2 - AC'], index = 0, key = 'origem_verba_'+str(n_acao))
            st.selectbox('ZBB', ['1 - CERV', '2 - NAB', '3 - MATCH', '4 - MKTPLACE'], index = 1,  key = 'zbb_verba_'+str(n_acao))
            st.selectbox('Canal', ['1 - ASR', '2 - ON TRADE', '3 - Central de Bebidas'], index = 1, key = 'canal_verba_'+str(n_acao))
            st.selectbox('Iniciativa', natural_sort(depara_acoes['depara_acao'].unique()), key = 'acoes_verba'+str(n_acao))
             
        with col6:

            st.metric('Clientes', clientes.shape[0])
            st.metric('Skus', produtos.shape[0])
            st.file_uploader('Importar Base Fechada (CSV com UNB, PDV)', key='base_fechada_'+str(n_acao), accept_multiple_files=False, type=['csv'])  
                
            if st.session_state['base_fechada_'+str(n_acao)]:
                base = pl.read_csv(st.session_state['base_fechada_'+str(n_acao)], encoding='latin1', infer_schema_length=200, sep=';')
...
def criar_template_cora_acao(session, clientes: pl.DataFrame, depara_ações: pl.DataFrame):
    all_dfs = []
    
    n_acao = sorted([int(x.split('_')[-1]) for x in session.keys() if x.startswith('nome_acao')])
    for n in n_acao:
        clientes_local = clientes.clone()
        filters = {
            name + '_' + str(n) : [name, session[name + '_' + str(n)]]
            for name in  ['comercial',
    'operacao',
    'seg_consolidado',
    'segmento'] if session[name + '_' + str(n)]
        }
        ...
        clientes_local = clientes_local.select(['unb', 'cod_pdv']).sort(['unb', 'cod_pdv']).with_columns([
            pl.lit(skus_selecionados).alias('cod_sku'),
            pl.lit('_'.join([session['origem_verba_' + str(n)].split(' - ')[0],
            ...
            [pl.lit(session[x[1] + str(n)]).alias(x[0]) for x in [
                ('titulo', 'nome_acao_'),
                ...
                pl.col('data_fim').dt.offset_by('7d').alias('data_entrega')
                ]).with_columns([
                    pl.col(name).dt.strftime("%d/%m/%Y") for name in ['data_inicio', 'data_fim', 'data_entrega']
                    ]).select(['agrupador', 'titulo', 'descricao', 'unb', 'cod_pdv', 'cod_sku', 'ttv_fixo', 'desconto_percentual', 'data_inicio', 'data_fim', 'data_entrega', 'min_sku', 'max_sku', 'quantidade_maxima_skus_promocao', 'disponibilidade_pedido', 'disponibilidade_sku', 'promo_justificativa', 'id_iniciativa'])
...
def create_promos_from_file(session):
    file = st.file_uploader('Importar Base Fechada (CSV com UNB, PDV)', key='base_fechada_'+str(0), accept_multiple_files=False, type=['csv'])
