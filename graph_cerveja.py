...
st.header('Resumo Repasse Cerveja - Jan/24')
...
repasse = repasse.with_columns(
    pl.when(pl.col("delta ttc") > 0).then(pl.lit('#d42e11'))\
      .when(pl.col("delta ttc") < 0).then(pl.lit('#32a852'))\
      .otherwise(pl.lit('#000000')).alias('color'),
)
repasse = repasse.select(comum + canais.get('BAR') + ['color']).to_pandas()
